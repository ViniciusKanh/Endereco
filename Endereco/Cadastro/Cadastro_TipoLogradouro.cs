...
namespace Endereco.Cadastro
{
    public partial class Cadastro_TipoLogradouro : Form
    {
        ...
        {
            this.dataLista.DataSource = TipoLogradouro.Buscar(this.txt_Buscar.Text);
            //  this.OcultarColunas();
        ...
        {
            this.dataLista.DataSource = TipoLogradouro.Mostrar();
            this.OcultarColunas();
            lbl_total.Text = "Total de Tipo de Logradouros " + Convert.ToString(dataLista.Rows.Count);
        }
                ...
                if (Opcao == DialogResult.OK)
                {
                    ...
                    foreach (DataGridViewRow row in dataLista.Rows)
                    {
                        if (Convert.ToBoolean(row.Cells[0].Value))
                        {
                            ...
                            Resp = TipoLogradouro.Remover(Convert.ToInt32(Codigo));
        ...
        private void btn_Salvar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    if (this.eNovo)
                    {
                        Resp = TipoLogradouro.Inserir(txt_Nome.Text.Trim());
                    }
                    else
                    {
                        Resp = TipoLogradouro.Editar(Convert.ToInt32(txt_IDTipoLogradouro.Text), txt_Nome.Text.Trim());
                    }
