...
{
    public partial class Cadastro_Logradouro : Form
    {
        private bool eNovo = false;
        private bool eEditar = false;
        public Cadastro_Logradouro()
        {
            InitializeComponent();
            this.ttMensagem.SetToolTip(this.txt_Nome, "Inserir nome de um Logradouro");
        }
        ...
        private void Limpar()
        {
            this.txt_IDLogradouro.Text = string.Empty;
        ...
        {
            this.txt_IDLogradouro.ReadOnly = !valor;
        ...
        {
            this.dataLista.DataSource = Logradouro.Buscar(this.txt_Buscar.Text);
            //  this.OcultarColunas();
        ...
        {
            this.dataLista.DataSource = Logradouro.Mostrar();
            this.OcultarColunas();
            lbl_total.Text = "Total de Logradouros " + Convert.ToString(dataLista.Rows.Count);
        }
        ...
        private void txt_IDLogradouro_TextChanged(object sender, EventArgs e)
        {
        ...
        private void btn_Salvar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    if (this.eNovo)
                    {
                        Resp = Logradouro.Inserir(txt_Nome.Text.Trim());
                    }
                    else
                    {
                        Resp = Logradouro.Editar(Convert.ToInt32(txt_IDLogradouro.Text), txt_Nome.Text.Trim());
                    }
        ...
        private void btn_Deletar_Click(object sender, EventArgs e)
        {
            ...
            {
                DialogResult Opcao;
                Opcao = MessageBox.Show("Deseja Realmente Apagar esse Logradouro", "BusSpott", MessageBoxButtons.OKCancel, MessageBoxIcon.Question);
                if (Opcao == DialogResult.OK)
                {
                    ...
                    foreach (DataGridViewRow row in dataLista.Rows)
                    {
                        if (Convert.ToBoolean(row.Cells[0].Value))
                        {
                            ...
                            Resp = Logradouro.Remover(Convert.ToInt32(Codigo));
        ...
        {
            this.txt_IDLogradouro.Text = Convert.ToString(this.dataLista.CurrentRow.Cells["ID"].Value);
            this.txt_Nome.Text = Convert.ToString(this.dataLista.CurrentRow.Cells["Nome"].Value);
            this.tabControl1.SelectedIndex = 1;
        }

        private void Cadastro_Logradouro_Load(object sender, EventArgs e)
        {
            this.Top = 0;
            this.Left = 0;
            this.Mostrar();
