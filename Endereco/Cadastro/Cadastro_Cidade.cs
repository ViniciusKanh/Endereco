...
namespace Endereco.Cadastro
{
    public partial class Cadastro_Cidade : Form
    {
        ...
        {
            MessageBox.Show(Mensagem, "Endereco", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
        ...
        {
            this.dataLista.DataSource = Classes.Cidade.Mostrar();
            this.OcultarColunas();
        ...
        private void btn_Salvar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    if (this.eNovo)
                    {
                        Resp = Classes.Cidade.Inserir(txt_Nome.Text.Trim(), Convert.ToInt32(cb_IDESTADO.SelectedValue), txt_CEP.Text);
                    }
                    else
                    {
                        Resp = Classes.Cidade.Editar(Convert.ToInt32(txt_IDCidade.Text), txt_Nome.Text.Trim(), Convert.ToInt32(cb_IDESTADO.SelectedValue), txt_CEP.Text);
                    }
                ...
                if (Opcao == DialogResult.OK)
                {
                    ...
                    foreach (DataGridViewRow row in dataLista.Rows)
                    {
                        if (Convert.ToBoolean(row.Cells[0].Value))
                        {
                            ...
                            Resp = Classes.Cidade.Remover(Convert.ToInt32(Codigo));
        ...
        private void Cadastro_Cidade_Load(object sender, EventArgs e)
        {
            ...
            cb_IDESTADO.DataSource = ConexaoBD.dql(sqlq);
            cb_IDESTADO.DisplayMember = "Sigla";
            cb_IDESTADO.ValueMember = "ID";
        }
        private void BuscarCidade()
        {
            this.dataLista.DataSource = Classes.Cidade.Buscar(this.txt_Buscar.Text);
            //  this.OcultarColunas();
