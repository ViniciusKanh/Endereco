...
namespace BusSpot.Cadastro
{
    public partial class Cadastro_Estado : Form
    {
        ...
        {
            this.dataLista.DataSource = Classes.Estado.Mostrar();
            this.OcultarColunas();
        ...
        {
            this.dataLista.DataSource = Classes.Estado.Buscar(this.txt_Buscar.Text);
          //  this.OcultarColunas();
            lbl_total.Text = "Total de Estados " + Convert.ToString(dataLista.Rows.Count);
        }
                ...
                if(Opcao == DialogResult.OK)
                {
                    ...
                    foreach(DataGridViewRow row in dataLista.Rows)
                    {
                        if (Convert.ToBoolean(row.Cells[0].Value))
                        {
                            ...
                            Resp = Classes.Estado.Remover(Convert.ToInt32(Codigo));
        ...
        private void btn_Salvar_Click_1(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    if (this.eNovo)
                    {
                        Resp = Classes.Estado.Inserir(txt_Nome.Text.Trim(), txt_Sigla.Text);
                    }
                    else
                    {
                        Resp = Classes.Estado.Editar(Convert.ToInt32(txt_IDESTADO.Text),txt_Nome.Text.Trim(), txt_Sigla.Text.ToUpper());
                    }
